        ...
        try:
            ...
            if "429" in error_str or "quota" in error_str.lower() or "Quota exceeded" in error_str:
                if attempt < max_retries - 1:
                    # リトライまでの待機時間を計算（エラーメッセージから取得、またはデフォルト60秒）
                    wait_time = 60
                    if "retry in" in error_str.lower():
                        # エラーメッセージから待機時間を抽出（例: "Please retry in 48.37s"）
                        import re
                        match = re.search(r'retry in ([\d.]+)s', error_str, re.IGNORECASE)
                        if match:
                            wait_time = int(float(match.group(1))) + 10  # 少し余裕を持たせる
                ...
                if attempt < max_retries - 1:
                    wait_time = 10 * (attempt + 1)  # 指数バックオフ
...
def main():
    ...
    for idx, work in enumerate(ranking_data[:max_articles], 1):

...
def generate_cursor_prompt(product_info: dict, input_url: str, user_features: str = "", example_articles: list[str] = None) -> str:
    ...
    if example_articles:
        ...
        example_section += "**特に、以下のスタイルも参考にしてください：**\n"
        example_section += "- 官能小説的な表現で、想像力を掻き立てる描写\n"
        example_section += "- 映画レビューのような視点で、ストーリーや演技を分析\n"
        example_section += "- 品のある言葉遣いで、余韻を残す文章\n"
        example_section += "- 詩的な表現で、うっとりするような読後感\n"
        example_section += "- 成熟した作品の魅力を、洗練された言葉で語る\n\n"
    ...
    prompt = f"""# 依頼
以下の作品データ（特に「紹介文」と「作品特徴」）を元に、官能小説のような品のある表現で、映画レビューのような視点から記事を書いてください。
成熟した作品の魅力を、洗練された言葉で語ってください。
...
# Role
あなたは成熟した作品を愛する、洗練された視聴者です。
熟女・人妻・ドラマ作品の魅力を、官能小説のような表現と映画レビューのような視点で語る、品のある文章を書いてください。

# スタイル指針
- **官能小説的な表現**: 直接的すぎず、想像力を掻き立てる表現を使用してください。
- **品のある言葉遣い**: 下品な表現やスラング（「ｗ」「マジで」「ヤバい」など）は一切使用禁止です。
- **映画レビューのような視点**: ストーリー構成、演出、演技を分析する視点で書いてください。
- **うっとりするような文章**: 余韻を残す、詩的な表現を心がけてください。
- **洗練された言葉**: 丁寧語は使わず、しかし品のある言葉で書いてください。
- **スペック（画角、解像度、制作年）などの説明は一切不要**です。
- **語尾のバリエーション**: 自然な語尾の変化で、文章にリズムを持たせてください。
- **トーンのバリエーション**: 
  - 時には静かに、じっくりと作品の魅力を語る
  - 時には情熱的に、作品への愛を表現する
  - 時には分析的に、ストーリーや演技を評価する
  - 時には情緒的に、作品がもたらす余韻を語る
...
2. **官能小説的な描写**: 
   - 直接的すぎず、想像力を掻き立てる表現を使用してください。
   - 品のある言葉で、シーンの美しさや情熱を描写してください。
   - 例：「教室の机の上で、セーラー服のボタンが静かに外されていく」のように、情緒的に。
   
3. **映画レビューのような視点**: 
   - ストーリー構成、演出、演技を分析する視点で書いてください。
   - 物語の展開や、女優の演技の妙を具体的に評価してください。
   - ネタバレしない範囲で、物語の核心に迫ってください。
   
4. **表現のバリエーション**: 
   - **重要：同じワードやフレーズを繰り返し使わないでください。**
   - 同じ意味を伝える場合でも、**構造や言い回しを大きく変えて**別の表現を使ってください。
   - 品のある言葉で、様々な表現を使ってください。
   
5. **主観的な感想を大切に**: 
   - 作品への個人的な感想を、洗練された言葉で語ってください。
   - 作品がもたらす余韻や、心に残るシーンを具体的に描写してください。
   
6. **詩的な表現の参考**: 
   - 季節や天気、場所などに絡めた情緒的な表現を適宜使用してください。
   - ただし、過度に装飾的にならず、自然に組み込んでください。
...
### 1. 詩的なタイトル
（「心を揺さぶる、禁断の物語」「大人の色気が香り立つ一本」など、作品の魅力を一言で表現する詩的なタイトル）
...
（女優の見た目、雰囲気、最初のシーンの印象を、品のある言葉で描写）
```

### 5. 物語の魅力
**重要**: このセクションでは、上記の「紹介文」に書かれている具体的なシーンやシチュエーションを必ず反映してください。
- ストーリーの構成、設定、展開を映画レビューのように分析してください。
- ネタバレしない範囲で、物語の核心に迫ってください。
- 紹介文から読み取れる内容を中心に、そのシーンの具体的な様子を、官能小説的な表現で描写してください。
- 例：「学校の教室でセーラー服を着たまま」→「教室の机の上で、セーラー服のボタンが静かに外されていく」のように、情緒的に。

### 6. 演技と演出の妙
- 女優の演技力、表情、仕草を具体的に評価してください。
- 監督の演出の工夫や、シーンの構成の美しさを分析してください。
- 作品がもたらす余韻や、心に残るシーンを具体的に描写してください。
...
**【文章の構成】**
1. **価格への言及（損得勘定を刺激）:**
   「この年代の作品を定価で購入するのは、少し贅沢かもしれません」
   「内容は素晴らしいものの、価格を考えると躊躇してしまう方もいるでしょう」

2. **サブスクの提案（解決策として自然に誘導）:**
   「DMM TV（またはFANZA見放題）の会員なら、追加料金なしで視聴できます」
   「まだ会員でない方は、**無料体験**を利用することをお勧めします」
   「匿名性も保たれ、安心して視聴できます」

3. **強烈なひと押し:**
   「この作品の価値を考えると、サブスクの会費は十分に元が取れるでしょう」
   「ランチ1回分の価格で、この時代の名作が数万本見放題というのは、現代の恩恵と言えるでしょう」

### 8. 読者への語りかけ（結び）
（作品の魅力を再確認し、読者に静かに語りかける結び）
（「この作品は、成熟した作品を愛する方にぜひ観ていただきたい一本です」と推す）
（作品がもたらす余韻や、心に残るシーンを再び語り、読者の心に響く結び）
...
- **品のある言葉遣いで、官能小説的な表現を使用してください。**
- **「ｗ」「マジで」「ヤバい」などのスラングは一切使用禁止です。**
- スペック（画角、解像度など）の説明は一切不要です。
- シーンは具体的に、しかし品のある言葉で描写してください。
- 主観的な感想を、洗練された言葉で語ってください。
...
- **重要：同じ記事内で同じワードやフレーズを繰り返し使わないでください。** 同じ意味を伝える場合でも、**構造や言い回しを大きく変えて**別の表現を使ってください。
